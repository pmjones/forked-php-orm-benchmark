...
class YiiMArrayHydrateTestSuite extends YiiMWithCacheTestSuite
{
    public function runHydrate($i)
    {
        $books = \app\models\Book::find()->from('Book b')->where('b.price > ?', [$i])->limit(5)->asArray()->all();
...
//        $this->em->clear();
    ...
    {
        $book = \app\models\Book::find()->from('Book b')->with('author')->where('b.title = ?', ['Hello' . $i])->limit(1)->asArray()->one();
